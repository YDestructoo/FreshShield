...
export default function DashboardScreen() {
  ...
  return (
    <ScreenContainer>
      <Animated.ScrollView showsVerticalScrollIndicator={false} className="flex-1" contentContainerClassName={status && connected ? 'gap-4 pb-[120px] pt-3' : 'flex-grow gap-4 pb-[120px] pt-3'} onScroll={onScroll} refreshControl={<RefreshControl refreshing={refreshing} onRefresh={() => void onRefresh()} tintColor="#F0525E" colors={['#F0525E']} />} scrollEventThrottle={16}>
      ...
      <Modal animationType="slide" transparent visible={detailsOpen} onRequestClose={() => setDetailsOpen(false)}>
        <Pressable className="flex-1 justify-end bg-black/30" onPress={() => setDetailsOpen(false)}><Pressable onPress={() => undefined} className="rounded-t-[30px] bg-[#F5F7F8] p-6">
